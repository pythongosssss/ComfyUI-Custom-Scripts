...
app.registerExtension({
	...
	setup() {
		...
		const feedFilter = app.ui.settings.addSetting({
			id: "pysssss.ImageFeed.FilterTempFiles",
			name: "🐍 Exclude Temp/Preview Images from Feed",
			//defaultValue: filterTempFiles,
			defaultValue: getVal("Filter", true),
			type: "boolean",
			onChange(value) {
				filterTempFiles = value;
				saveVal("Filter", value);
			},
		});
		...
		api.addEventListener("executed", ({ detail }) => {
			if (visible && detail?.output?.images) {
				...
				for (const src of detail.output.images) {
					if (filterTempFiles == true && src.filename.includes("temp")) {
						// Filter files named 'temp' and anything from a Preview Node (which names them temp).
						continue;
					}
